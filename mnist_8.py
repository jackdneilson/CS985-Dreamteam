...
def nn_network(combination, learning_rate, epochs, batches, seed):
    ...
    if combination == 1 or combination == 3 or combination == 5 or combination == 6:
...
def build_model(combination, learning_rate):
    ...
    if combination == 5:
        model = cnn(combination, model, activation = leaky_relu)
    
    if combination == 6:
        model = cnn(combination, model, activation = tf.nn.elu)

    if combination == 7:
        model = mlp(combination, model, activation=tf.nn.sigmoid)
        
    if combination == 8:
        model = mlp(combination, model, activation = tf.nn.tanh)        
...
def mlp(combination, model, activation):
    ...
    if combination == 4 or combination == 7:
...
# Combination 5: CNN with 1 hidden layer leaky_relu
# Combination 6: CNN with 1 hidden layer elu
# Combination 7: MLP with 2 hidden layers sigmoid GDO
# Combination 8: MLP with 2 hidden layers tanh

# Run a neural network with the given parameters
nn_network(6, 0.003, 6, 128, 12345)
